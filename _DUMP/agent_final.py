...
def write_file(generation, best_cost, filename='generation_cost.txt'):
    with open(filename, 'a') as file:
        file.write(f'{generation},{best_cost}\n')
...
class Agent:
    ...
    def evalF(self,chromosome):
        ...
        while True:
            ...
            cost =  self.overshoot_error*self.v + self.game_time/120

            if self.game.RESET == True:
                break
        if self.game.current_life == self.game.START_CAPTURE_LIFE:
            cost = cost*900 + self.relative_states[0]
    ...
    def get_state(self):
        ...
        if self.game.current_life < self.game.START_CAPTURE_LIFE:
            self.overshoot_error = position_error if position_error > self.overshoot_error else self.overshoot_error
        else:
            self.game_time = self.game.ticks
        return
    ...
    def FIS(self, chromosome):
        ...
        write_file(self.game_time,self.v,'velocity_time')
        write_file(self.game_time,pe,'positionError_time')
...
def play():
    ...
    while True:
        ...
        chromosome = [0, 4, 4, 1, 3, 3, 3, 0, 1, 3, 4, 2, 0, 0, 0, 1, 4, 1, 2, 3, 3, 2, 0, 1, 1, 3, 0, 2, 1, 1, 4, 1, 2, 2, 0, 1, 4, 3, 2, 0, 2, 0, 2, 3, 2, 3, 4, 0, 1, 0, 4, 4]
        #get move
