...
class Navigation:
    ...
    def choose_pseudo_target(self, game_state):
        ...
        #Asteroid states
        asteroid_clearance_list = []
        asteroid_bearing_list = []
        asteroid_distance_list = []
        for i, a in enumerate(game_state['asteroids']):
            ...
            asteroid_clearance_list.append(asteroid_clearance)
            asteroid_bearing_list.append(asteroid_bearing)
            asteroid_distance_list.append(asteroid_distance)
        ...
        avoidance_offset = avoidance_out["avoidance_offset"]
        ...
        asteroid_along_velcoity = to_asteroid.dot(peach_velocity_direction)
        closest_point = peach_position + peach_velocity_direction*asteroid_along_velcoity
        cross_track_distance = (game_state['asteroids'][0]['position'] - closest_point).length()
        asterdoid_on_velcoty_path = (asteroid_along_velcoity > 0 and cross_track_distance < effective_radius)
        
        if asteroid_offset > target_distance:
            avoidance_offset = 0
